module.exports = function (io, db_api) {
	...
	var fn_api = require(root_folder + '/backend/functions/');
		...
		socket.on('auth', function (data, fn) {
			...
			.on('signin', function (v, fn) {
				...
				db_api.userSignin({
					...
					var token = jwt.sign({ uid: ok._id }, config.token_secret, { expiresIn: '10d' });
					fn({ token: token });
				});
			})
			...
			.on('change_pass', function (v, fn) {
				...
				db_api.userFindByEmail(email, fail => {
					...
					db_api.restoreemailFindRecent({
						...
						var hashpas = require(root_folder + '/requires/crypto-hash')(v.newpass + config.server_salt);
